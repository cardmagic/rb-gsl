...
static VALUE rb_gsl_multiroot_function_new(int argc, VALUE *argv, VALUE klass)
{
  ...
  return TypedData_Wrap_Struct(klass, &gsl_multiroot_function_data_type, F);
}
...
static int rb_gsl_multiroot_function_f(const gsl_vector *x, void *p, gsl_vector *f)
{
  VALUE vx, vf;
  VALUE vp, proc;
  /* Use tmp type: x and f are owned by GSL and must not be freed by Ruby */
  vx = TypedData_Wrap_Struct(cgsl_vector, &gsl_vector_tmp_data_type, (gsl_vector *) x);
  vf = TypedData_Wrap_Struct(cgsl_vector, &gsl_vector_tmp_data_type, f);
...
static VALUE rb_gsl_multiroot_function_fdf_new(int argc, VALUE *argv, VALUE klass)
{
  ...
  return TypedData_Wrap_Struct(klass, &gsl_multiroot_function_fdf_data_type, F);
}
                                            ...
                                            gsl_matrix *J)
{
  VALUE vx, vJ, ary;
  VALUE proc, vp;
  /* Use tmp type: x and J are owned by GSL and must not be freed by Ruby */
  vx = TypedData_Wrap_Struct(cgsl_vector, &gsl_vector_tmp_data_type, (gsl_vector *) x);
  vJ = TypedData_Wrap_Struct(cgsl_matrix, &gsl_matrix_tmp_data_type, J);
  ary = (VALUE) p;
                                             ...
                                             gsl_vector *f, gsl_matrix *J)
{
  ...
  /* Use tmp type: x, f, and J are owned by GSL and must not be freed by Ruby */
  vx = TypedData_Wrap_Struct(cgsl_vector, &gsl_vector_tmp_data_type, (gsl_vector *) x);
  vf = TypedData_Wrap_Struct(cgsl_vector, &gsl_vector_tmp_data_type, f);
  vJ = TypedData_Wrap_Struct(cgsl_matrix, &gsl_matrix_tmp_data_type, J);
  ary = (VALUE) p;
...
static VALUE rb_gsl_multiroot_fsolver_fsolve(int argc, VALUE *argv, VALUE obj)
{
  ...
  return rb_ary_new3(3,
                     TypedData_Wrap_Struct(cgsl_vector, &gsl_vector_data_type, xnew),
...
static VALUE rb_gsl_multiroot_fdjacobian(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (argc == 4) {
    ...
    return rb_ary_new3(2, TypedData_Wrap_Struct(cgsl_matrix, &gsl_matrix_data_type, J),
                       INT2FIX(status));
  } else {
    TypedData_Get_Struct(argv[4], gsl_matrix, &gsl_matrix_data_type, J);
    status = gsl_multiroot_fdjacobian(F, x, f, eps, J);
...
static VALUE rb_gsl_multiroot_function_solve(int argc, VALUE *argv, VALUE obj)
{
  ...
  return rb_ary_new3(3, TypedData_Wrap_Struct(cgsl_vector, &gsl_vector_data_type, xnew),
                     INT2FIX(iter), INT2FIX(status));
}
