...
static VALUE rb_gsl_histogram3d_new(int argc, VALUE *argv, VALUE klass)
{
  ...
  case 3:
    if (TYPE(argv[0]) == T_FIXNUM
      ...
      TypedData_Get_Struct(argv[0], gsl_vector, &gsl_vector_data_type, xrange);
      TypedData_Get_Struct(argv[1], gsl_vector, &gsl_vector_data_type, yrange);
      TypedData_Get_Struct(argv[2], gsl_vector, &gsl_vector_data_type, zrange);
  ...
  return TypedData_Wrap_Struct(cgsl_histogram3d, &gsl_histogram3d_data_type, h);
}

static VALUE rb_gsl_histogram3d_nx(VALUE obj)
{
  mygsl_histogram3d *h;
  TypedData_Get_Struct(obj, mygsl_histogram3d, &gsl_histogram3d_data_type, h);
  return INT2FIX(h->nx);
}
...
static VALUE rb_gsl_histogram3d_get(int argc, VALUE *argv, VALUE obj)
{
  ...
  case 1:
    ...
    case T_FIXNUM:
      ...
      return TypedData_Wrap_Struct(cgsl_histogram3d_view, &gsl_histogram3d_view_data_type, h2);
      break;
    case T_ARRAY:
...
static VALUE rb_gsl_histogram3d_set_ranges(VALUE obj, VALUE xx, VALUE yy, VALUE zz)
{
  ...
  if (TYPE(xx) == T_ARRAY) {
    ...
    TypedData_Get_Struct(xx, gsl_vector, &gsl_vector_data_type, xrange);
  } else {
  ...
  if (TYPE(yy) == T_ARRAY) {
    ...
    TypedData_Get_Struct(yy, gsl_vector, &gsl_vector_data_type, yrange);
  } else {
    rb_raise(rb_eTypeError, "wrong argument type %s (Array or Vector expected)",
  ...
  if (TYPE(zz) == T_ARRAY) {
    ...
    TypedData_Get_Struct(zz, gsl_vector, &gsl_vector_data_type, zrange);
  } else {
    rb_raise(rb_eTypeError, "wrong argument type %s (Array or Vector expected)",
...
static VALUE rb_gsl_histogram3d_memcpy(VALUE obj, VALUE a, VALUE b)
{
  ...
  TypedData_Get_Struct(a, mygsl_histogram3d, &gsl_histogram3d_data_type, dst);
  TypedData_Get_Struct(b, mygsl_histogram3d, &gsl_histogram3d_data_type, src);
...
static VALUE rb_gsl_histogram3d_xyproject(int argc, VALUE *argv, VALUE obj)
{
  ...
  TypedData_Get_Struct(obj, mygsl_histogram3d, &gsl_histogram3d_data_type, h3);
  switch (argc) {
  case 2:
  ...
  return TypedData_Wrap_Struct(cgsl_histogram2d, &gsl_histogram2d_data_type, h2);
}
                                     ...
                                     int (*func)(mygsl_histogram3d *, const mygsl_histogram3d*))
{
  ...
  TypedData_Get_Struct(obj, mygsl_histogram3d, &gsl_histogram3d_data_type, h1);
